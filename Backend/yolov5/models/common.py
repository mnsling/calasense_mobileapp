...
def _resize_like(x, ref):
    # nearest for speed & to match typical YOLOv5 upsample behavior
    return F.interpolate(x, size=ref.shape[-2:], mode='nearest')

class WeightedAdd(nn.Module):
    """
    Learnable positive weights with fast softplus to keep >0
    """
...
class BiFPNLayer(nn.Module):
    """
    One BiFPN layer over 3 pyramid levels (P3, P4, P5).
    Inputs: [P3_in, P4_in, P5_in]
    Outputs: [P3_out, P4_out, P5_out]
    c_out3/4/5 may differ (e.g., 256/512/512).
    """
    def __init__(self, c3, c4, c5, c_out3, c_out4, c_out5, sep=True, eps=1e-4):
        super().__init__()
        Conv = SeparableConv2d if sep else nn.Conv2d

        # Lateral projections -> target channels
        self.l3 = nn.Conv2d(c3, c_out3, 1, 1, 0, bias=False)
        self.l4 = nn.Conv2d(c4, c_out4, 1, 1, 0, bias=False)
        self.l5 = nn.Conv2d(c5, c_out5, 1, 1, 0, bias=False)

        # Learnable-weighted sum blocks
        self.td4_w = WeightedAdd(2, eps)
        self.td3_w = WeightedAdd(2, eps)
        self.bu4_w = WeightedAdd(3, eps)   # P4_l + P4_td + down(P3_td)
        self.bu5_w = WeightedAdd(2, eps)   # P5_l + down(P4_bu)

        # Convs after fusion
        self.td4_conv = Conv(c_out4, c_out4, 3, 1)
        self.td3_conv = Conv(c_out3, c_out3, 3, 1)
        self.bu4_conv = Conv(c_out4, c_out4, 3, 1)
        self.bu5_conv = Conv(c_out5, c_out5, 3, 1)

        # Channel adapters for asymmetric widths
        self.adapt_P4toP3 = nn.Conv2d(c_out4, c_out3, 1, 1, 0, bias=False) if c_out4 != c_out3 else nn.Identity()
        self.adapt_P3toP4 = nn.Conv2d(c_out3, c_out4, 1, 1, 0, bias=False) if c_out3 != c_out4 else nn.Identity()
        self.adapt_P4toP5 = nn.Conv2d(c_out4, c_out5, 1, 1, 0, bias=False) if c_out4 != c_out5 else nn.Identity()

        self.bn3 = nn.BatchNorm2d(c_out3)
        self.bn4 = nn.BatchNorm2d(c_out4)
        self.bn5 = nn.BatchNorm2d(c_out5)
        self.act = nn.SiLU()

    @staticmethod
    def _resize(x, ref):
        # Always match HxW to ref; works for up/down using nearest (YOLO-style)
        return F.interpolate(x, size=ref.shape[-2:], mode='nearest')

    def forward(self, xs):
        assert len(xs) == 3, "BiFPNLayer expects [P3,P4,P5]"
        P3_in, P4_in, P5_in = xs

        # Lateral
        P3_l = self.l3(P3_in)  # [B, c3o, H3, W3]
        P4_l = self.l4(P4_in)  # [B, c4o, H4, W4]
        P5_l = self.l5(P5_in)  # [B, c5o, H5, W5]

        # ---------- Top-down ----------
        P5_td = P5_l  # top level unchanged
        # P4_td: fuse P4_l with up(P5_td)
        P5_up = self._resize(P5_td, P4_l)         # [B, c5o, H4, W4]
        P4_td_in = self.td4_w([P4_l, P5_up])
        P4_td = self.td4_conv(P4_td_in)           # [B, c4o, H4, W4]

        # P3_td: fuse P3_l with up(P4_td), channel-adapt if needed
        P4_up = self._resize(P4_td, P3_l)         # [B, c4o, H3, W3]
        P4_up = self.adapt_P4toP3(P4_up)          # [B, c3o, H3, W3]
        P3_td_in = self.td3_w([P3_l, P4_up])
        P3_td = self.td3_conv(P3_td_in)           # [B, c3o, H3, W3]

        # ---------- Bottom-up ----------
        # P4_bu: fuse P4_l, P4_td, down(P3_td)
        P3_dn = self._resize(P3_td, P4_l)         # downsample/upsample to H4,W4 and c3o
        P3_dn = self.adapt_P3toP4(P3_dn)          # [B, c4o, H4, W4]
        P4_bu_in = self.bu4_w([P4_l, P4_td, P3_dn])
        P4_bu = self.bu4_conv(P4_bu_in)           # [B, c4o, H4, W4]

        # P5_bu: fuse P5_l, down(P4_bu)
        P4_dn = self._resize(P4_bu, P5_l)         # [B, c4o, H5, W5]
        P4_dn = self.adapt_P4toP5(P4_dn)          # [B, c5o, H5, W5]
        P5_bu_in = self.bu5_w([P5_l, P4_dn])
        P5_bu = self.bu5_conv(P5_bu_in)           # [B, c5o, H5, W5]

        # Norm + act
        return self.act(self.bn3(P3_td)), self.act(self.bn4(P4_bu)), self.act(self.bn5(P5_bu))

class BiFPN(nn.Module):
    """
    Stacks multiple BiFPNLayer blocks and returns 3 outputs for Detect.
    """
    def __init__(self, c_in, c_out=None, n=3, sep=True):
        """
        c_in: list/tuple of input channels [c3, c4, c5]
        c_out: list/tuple target out channels [o3, o4, o5] (default: match c_in[0]/[1]/[2])
        n: number of BiFPN layers to stack
        """
        super().__init__()
        assert isinstance(c_in, (list, tuple)) and len(c_in) == 3
        if c_out is None:
            c_out = c_in
        assert len(c_out) == 3
...
class CoordAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        a_h, a_w = self.conv_h(x_h).sigmoid(), self.conv_w(x_w).sigmoid()
        return x * a_h * a_w
...
class C3CA(C3):
    ...
    def forward(self, x):
        ...
        if self.attn is None:
            c_out = y.shape[1]
            self.attn = CoordAttention(c_out, c_out).to(y.device)
        return self.attn(y)
...
class C3CBAM(C3):
    ...
    def forward(self, x):
        ...
        if self.attn is None:
            c_out = y.shape[1]
            self.attn = CBAM(c_out).to(y.device)
        return self.attn(y)
